        ...
        server.on('message', async (msg, rinfo) => {
            ...
            if (msg) {
                ...
                if (!Buffer.isBuffer(msg)) {
                    this.adapter.log.warn(`[UDP] Received non-Buffer message from ${rinfo.address}:${rinfo.port}, type: ${typeof msg}`);
                    return;
                }
                ...
                if (!msgString.trim().startsWith('{') || !msgString.trim().endsWith('}')) {
                    this.adapter.log.warn(`[UDP] Received non-JSON format message from ${rinfo.address}:${rinfo.port}: "${msgString}"`);
                    return;
                }
                ...
                try {
                    ...
                    this.adapter.log.warn(`[UDP] JSON parse failed from ${rinfo.address}:${rinfo.port}: "${msgString}"`);
                ...
                if (typeof deveiceInfo !== 'object' || deveiceInfo === null || Array.isArray(deveiceInfo)) {
                    this.adapter.log.warn(`[UDP] Parsed JSON is not a valid object from ${rinfo.address}:${rinfo.port}: ${JSON.stringify(deveiceInfo)}`);
                    return;
                }
                
                if (!deveiceInfo.devName || !deveiceInfo.uuid) {
                    this.adapter.log.warn(`[UDP] Missing required fields in device info from ${rinfo.address}:${rinfo.port}: ${JSON.stringify(deveiceInfo)}`);
                    return;
                }
                // Verify devName and uuid
                const devName = this.sanitizeDevName(deveiceInfo.devName);
                if (!this.isValidUuid(deveiceInfo.uuid)) {
                    this.adapter.log.warn(`[UDP] Invalid UUID from ${rinfo.address}:${rinfo.port}: ${deveiceInfo.uuid}`);
                    return;
                }
                // Init Objects
                if (this.deviceExists(devName) && !this.deviceUuidList.includes(deveiceInfo.uuid)) {
                    ...
                    this.adapter.log.info(`[UDP] Adding new device: ${devName} (UUID: ${deveiceInfo.uuid}) from ${rinfo.address}:${rinfo.port}`);
                    ...
                    this.adapter.log.debug(`[UDP] Unsupported device type: ${devName} from ${rinfo.address}:${rinfo.port}`);
                } else {
                    this.adapter.log.debug(`[UDP] Device already exists: ${devName} (UUID: ${deveiceInfo.uuid}) from ${rinfo.address}:${rinfo.port}`);
                }
            }
        });
        // Server listening
