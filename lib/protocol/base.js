    ...
    async httpState() {
            ...
            try {
                ...
                if (!httpAck || typeof httpAck !== 'object') {
                    ...
                    this.adapter.log.warn(
                        `[httpState] Invalid HTTP response for ${this.getLogInfo()}, namespace: ${namespace}`,
                    );
                ...
                if (!httpAck[getAckKey]) {
                    failCount++;
                    this.adapter.log.warn(
                        `[httpState] Missing key '${getAckKey}' for ${this.getLogInfo()}, namespace: ${namespace}`,
                    );
                    continue; // 跳过本次更新，继续下一个namespace
                }
                ...
                if (!this.httpData) {
                    failCount++;
                    this.adapter.log.warn(
                        `[httpState] httpData is null/undefined for ${this.getLogInfo()}, namespace: ${namespace}`,
                    );
                    continue; // 跳过本次更新，继续下一个namespace
                }
                ...
                this.adapter.log.error(
                    `[httpState] Error for ${this.getLogInfo()}, namespace: ${namespace}: ${error.message || error}`,
                );
            }
        }
        ...
        if (failCount > 0 && successCount === 0) {
            ...
            this.adapter.log.debug(
                `[httpState] Polling summary for ${this.getLogInfo()}, success: ${successCount}, failed: ${failCount}`,
            );
        }
