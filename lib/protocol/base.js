    ...
    async httpState() {
        ...
        for (const namespace in this.http) {
            ...
            try {
                ...
                // 打印检查mConsumeX字段
                if (this.httpData && Array.isArray(this.httpData)) {
                    this.httpData.forEach((channelData, index) => {
                        if (channelData) {
                            const hasmConsumeX = 'mConsumeX' in channelData;
                            const mConsumeXValue = channelData['mConsumeX'];
                            this.adapter.log.info(`[检查mConsumeX] 通道${index + 1}: mConsumeX存在=${hasmConsumeX}, mConsumeX值=${mConsumeXValue}`);
                        }
                    });
                } else if (this.httpData) {
                    // 如果不是数组，直接检查
                    const hasmConsumeX = 'mConsumeX' in this.httpData;
                    const mConsumeXValue = this.httpData['mConsumeX'];
                    const mConsumeValue = this.httpData['mConsume'];
                    this.adapter.log.info(`[检查mConsumeX] 设备数据: mConsumeX存在=${hasmConsumeX}, mConsumeX值=${mConsumeXValue}, mConsume值=${mConsumeValue}`);
                }
    ...
    getElectricity(channel, type) {
        for (const i in this.httpData) {
            if (parseInt(i) + 1 == channel) {
                ...
                // 打印mConsumeX相关调试信息
                if (type === 'mConsumeX' || type === 'mConsume') {
                    const channelData = this.httpData[i];
                    const hasmConsumeX = channelData && 'mConsumeX' in channelData;
                    const mConsumeXValue = channelData ? channelData['mConsumeX'] : undefined;
                    const mConsumeValue = channelData ? channelData['mConsume'] : undefined;
                }
    ...
    async processChannelMergeRequest() {
        ...
        try {
            ...
            if (mergeChannelData && mergeChannelData.length > 0) {
                ...
                // *拼合并通道名称
                const mergeChannelNameMap = this.mergeChannelList.map(item => `refoss.0.${this.deviceId}.Total-${item.mergeName}`);
