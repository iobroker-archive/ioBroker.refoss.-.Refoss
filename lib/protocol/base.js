        ...
        for (const namespace in this.http) {
            ...
            try {
                ...
                if (this.httpData && Array.isArray(this.httpData)) {
                    this.httpData.forEach((channelData, index) => {
                        if (channelData) {
                            ...
                            this.adapter.log.info(
                                `[检查mConsumeX] 通道${index + 1}: mConsumeX存在=${hasmConsumeX}, mConsumeX值=${mConsumeXValue}`,
                            );
                        }
                    });
                    ...
                    this.adapter.log.info(
                        `[检查mConsumeX] 设备数据: mConsumeX存在=${hasmConsumeX}, mConsumeX值=${mConsumeXValue}, mConsume值=${mConsumeValue}`,
                    );
                }
                for (const i in dps) {
        ...
        for (const mergeChannel of this.mergeChannelList) {
            ...
            if (!mergeName) {
                this.adapter.log.warn(
                    `[createDynamicMergeChannelDataPoints] Merge channel without name: ${JSON.stringify(mergeChannel)}`,
                );
                continue;
            }
            ...
            if (existingOldObject) {
                ...
                this.adapter.log.info(
                    `[createDynamicMergeChannelDataPoints] Cleaned up old merge channel object: ${fullOldObjectStateId}`,
                );
            }
