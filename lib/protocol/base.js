        ...
        for (const namespace in this.http) {
            ...
            try {
                ...
                if (!httpAck || typeof httpAck !== 'object') {
                    failCount++;
                    this.adapter.log.warn(`[httpState] Invalid HTTP response for ${this.getLogInfo()}, namespace: ${namespace}`);
                ...
                if (!httpAck[getAckKey]) {
                    failCount++;
                    this.adapter.log.warn(`[httpState] Missing key '${getAckKey}' for ${this.getLogInfo()}, namespace: ${namespace}`);
                    continue; // 跳过本次更新，继续下一个namespace
                }
                ...
                if (!this.httpData) {
                    failCount++;
                    this.adapter.log.warn(`[httpState] httpData is null/undefined for ${this.getLogInfo()}, namespace: ${namespace}`);
                    continue; // 跳过本次更新，继续下一个namespace
                }
                ...
                failCount++;
                this.adapter.log.error(`[httpState] Error for ${this.getLogInfo()}, namespace: ${namespace}: ${error.message || error}`);
            }
        }

        // 记录轮询状态摘要（仅在有问题时记录）
        if (failCount > 0 && successCount === 0) {
            this.adapter.log.warn(`[httpState] All requests failed for ${this.getLogInfo()}, failed: ${failCount}`);
        } else if (failCount > 0) {
            this.adapter.log.debug(`[httpState] Polling summary for ${this.getLogInfo()}, success: ${successCount}, failed: ${failCount}`);
        }
